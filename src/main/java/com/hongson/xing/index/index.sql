...
#sql("getShareList")
	select s.id,
		...
		substring(s.content, 1, 180) as content, s.createAt as createAt, s.clickCount, s.likeCount,
		a.avatar,
		a.id as accountId
...
#sql("getFeedbackList")
	select f.id,
		...
		substring(f.content, 1, 180) as content, f.createAt as createAt, f.clickCount, f.likeCount,
		a.avatar,
		a.id as accountId
